...
const { addLeadingSlash } = require('./path-slashes');

/**
 * Takes a definition string and uses it to remove leading parts of a path if necessary.
 * The definition may include glob segments which, if present,
 * will be removed before use from the definition along with all trailing parts of the
 * definition string.
 *
...
module.exports = function pathDiff(definition, path) {
    ...
    const replaced = normalize(addLeadingSlash(path)).replace(matcher, '');
    return addLeadingSlash(replaced);
};
